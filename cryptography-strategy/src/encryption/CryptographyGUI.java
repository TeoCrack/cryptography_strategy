...
public class CryptographyGUI extends JPanel {
    ...
    //objects for scytale, copy and caesar
    private Caesar caesar = new Caesar();
    private Scytale scytale = new Scytale();
    private Copy copy = new Copy();
    
    
    //intializing selectedEncryption - a variable that keeps track of the selected algorithm.
    //this variable is intialized as COPY because without any selection, the first choice algorithm is COPY
    public String selectedEncryption = COPY; 
     ...
     */
    public String selectEncryption(String selected) {

        if (selected.equals(COPY)) {
            //stores the type of algorithm
            selectedEncryption = COPY;
    ...
    public String encryptWithKey(String plainText, int key) {
        ...
        if(selectedEncryption.equals(COPY)){
            //implements copy encryption algorithm
            return copy.encrypt(plainText, key);
        }
        
        else if(selectedEncryption.equals(CAESAR)) {
            //implements caesar encryption algorithm
            return caesar.encrypt(plainText, key);
        }
        
        else if(selectedEncryption.equals(SCYTALE)){
            //implements scytale encryption algorithm
            return scytale.encrypt(plainText, key);
            
        }
       
        //the first intialized selectedEncryption Algorithm is COPY
        //so it will return the plainText if there is't any selection
    ...
    public String decryptWithKey(String cipherText, int key) {
        ...
        if(selectedEncryption.equals(COPY)){
          //implements copy decryption algorithm
            return copy.decrypt(cipherText, key);
        }
        
        else if(selectedEncryption.equals(CAESAR)) {
          //implements caesar decryption algorithm
            return caesar.decrypt(cipherText, key);
        }
        else if(selectedEncryption.equals(SCYTALE)){
          //implements scytale decryption algorithm
            return scytale.decrypt(cipherText, key);
            
        }
        
        //the first intialized selectedEncryption Algorithm is COPY
        //so it will return the cipherText if there is't any selection
        //this might be redudant
        return cipherText;
    }
